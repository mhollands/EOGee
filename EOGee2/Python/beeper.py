...
def update(frame):
	...
	new_y_data = [x & 0x0FFF for x in points_16bit if (x >> 12) == 0x8]

	ydata.extend(new_y_data)
	[dacdata.append(x & 0x0FFF) for x in points_16bit if (x >> 12) == 0x4]

	dif = np.diff(new_y_data)
	if(np.max(np.abs(dif)) > 200):
		print("beep \a")

	y = ydata - (np.array(dacdata) - 2048)*adc_per_dac
	y = np.convolve(y, np.ones(104)/104, mode="valid")
